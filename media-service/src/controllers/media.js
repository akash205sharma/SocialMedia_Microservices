...
const getMediaWithId = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await Media.findOne({ publicId: id });

    if (!result || result.length === 0) {
      return res.status(404).json({
        success: false,
        message: "Cann't find this media"
      })
    }
...
module.exports = { uploadMedia, getAllMedias, getMediaWithId };